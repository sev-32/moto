...
class FiniteThicknessTyreSolverV5{
 constructor(asset,opt={}){
  ...
  let ths=[...new Set((asset.visualVertices||[]).filter(v=>v[4]===Math.floor(asset.profile.length/2)).map(v=>v[3]))].sort((a,b)=>a-b);this.thetaPhase=ths.length?ths[0]:0;this.sourceRingCount=ths.length||0;
  this.p=Object.assign({
   Ntheta:32, pressurePa:34.8*6894.757293168, ambientPa:101325, gasGamma:1.24,
   tyreMassKg:5.8, rimHubMassKg:9, axlePayloadKg:145, gravity:9.81, camberDeg:0,
   speedMps:0, slipAngleDeg:0, slipRatio:0, mu:1.18, roadZ:0, dt:1/720, constraintIterations:10,
  ...
  this.time=0;this.hubZ=asset.nominal.R0+.05;this.hubVz=0;this.inflatedReference=null;this.innerInflatedReference=null;this.Vasset=0;this.VgasRef=0;this.materialVolumeRef=0;this.dampingLossJ=0;this.brushLossJ=0;this.drop={};
  this.build();
 }
 ...
 buildVisualContacts(){
  ...
  let nt=this.p.Ntheta,np=this.outerProfile.length,out=[],sub=5;
  for(let it=0;it<nt;it++)for(let st=0;st<sub;st++){
   let t=(st+.5)/sub,ang=this.thetaPhase+2*PI*(it+t)/nt;
   for(let j=0;j<np;j++){
 ...
 solveConstraints(dt,opt={}){
  for(let c of this.constraints)c.lambda=0;for(let q of this.visualContacts)q.lambda=0;let hubInv=opt.holdHub?0:1/Math.max(1,this.p.axlePayloadKg+this.p.rimHubMassKg+this.beadMassKg),road=this.p.roadZ;
  for(let it=0;it<this.p.constraintIterations;it++){
   for(let ci=0;ci<this.constraints.length;ci++){let c=this.constraints[(it&1)?this.constraints.length-1-ci:ci],a=this.particles[c.a],b=this.particles[c.b],A=this.particlePos(a),B=this.particlePos(b),d=sub(B,A),L=len(d);if(L<1e-9)continue;let e=mul(d,1/L),C=L-c.L0,gaZ=a.bead?-e[2]:0,gbZ=b.bead?e[2]:0,gHub=gaZ+gbZ,w=(a.bead?0:a.invMass)+(b.bead?0:b.invMass)+hubInv*gHub*gHub;if(w<=0)continue;let alpha=1/(c.k*dt*dt),dl=(-C-alpha*c.lambda)/(w+alpha);c.lambda+=dl;if(!a.bead){a.pos[0]-=a.invMass*dl*e[0];a.pos[1]-=a.invMass*dl*e[1];a.pos[2]-=a.invMass*dl*e[2]}if(!b.bead){b.pos[0]+=b.invMass*dl*e[0];b.pos[1]+=b.invMass*dl*e[1];b.pos[2]+=b.invMass*dl*e[2]}if(hubInv&&gHub)this.hubZ+=hubInv*dl*gHub}
   this.enforceLateralSymmetry();
   // Road contact is only on the actual Ducati OUTER render surface.
  ...
  // final one-way projection; never attracts tire to road
 ...
 applyBrushForces(){
  ...
  let travel=Math.max(.005,lead-trail),g=this.camberRad(),lat=[Math.cos(g),0,-Math.sin(g)],ta=Math.tan(alpha);
  for(let n of contact){
   ...
   let refO=this.inflatedReference?this.transformLocal(this.inflatedReference[this.idx(n.it,n.j)]):this.transformLocal(n.local),
       ...
       shear=sub(sub(n.pos,inn.pos),sub(refO,refI)),uLong=shear[1],uLat=dot(shear,lat),
       targetLong=kappa*dist,targetLat=ta*dist,kb=P.brushDensity*n.area,
   ...
   n.force[1]+=fl;n.force[0]+=lat[0]*fy;n.force[2]+=lat[2]*fy;
  }
 }
 ...
 projectRenderSurface(){
  let road=this.p.roadZ,maxCorr=0,g=this.camberRad(),cg=Math.cos(g),sg=Math.sin(g),near=road+0.040;
 ...
 substep(dt=this.p.dt,opt={}){
  ...
  let pressureHubFz=this.addPressureForces();this.addCentrifugal();this.applyBrushForces();let M=P.axlePayloadKg+P.rimHubMassKg+this.beadMassKg;
  if(!opt.holdHub)this.hubZ+=this.hubVz*dt+(-P.gravity+(pressureHubFz+(opt.hubExternalForceN||0))/Math.max(1,M))*dt*dt;else this.hubVz=(opt.hubVz===undefined?0:opt.hubVz);
